...
It may be argued that time in hospital could be related to the effect of the treatment rather than determining it. 
...
diabetes_matched <- match.data(matched_list)

summary(matched_list2, un = FALSE)
plot(matched_list2, type = "jitter", interactive = FALSE)
diabetes_matched2 <- match.data(matched_list2)
```
...
model1 <- glm(readmit_yesno ~ medication + race + 
                diagnosis + time_in_hospital + 
                num_medications + dischargestatus + 
                admissionsource, 
              family = "binomial", 
              data = diabetes_matched)

summary(model1)

#fit model 2 for readmission <30 days vs. readmission >30 days
model2 <- glm(readmit_30days ~ medication + race + 
                diagnosis + time_in_hospital + 
                num_medications + dischargestatus + 
                admissionsource, 
              family = "binomial", 
              data = diabetes_matched2)
