...
tidy_m1 <- tidy(model1) %>%
  ...
  filter(term == c("medication", "race2", "race3", "race4"))
...
models2 <- rbind(tidy_m2, tidy_m2_sens) %>%
  mutate(term = case_when(term == "medication" ~ "Medication",
                          term == "race2" ~ "Black",
                          term == "race3" ~ "Hispanic",
                          term == "race4" ~ "Other"))
...
- Being prescribed metformin is associated with protection against readmission at all, and earlier readmission versus being prescribed insulin.

- Race was not a strong predictor of readmission or earlier readmission; race and medication as an interaction term did not show significant statistical effect modification.
...
- Causal mechanism model is based on may be incorrect
